          ...
          <div style="min-height: 80px" v-if="!finished" v-loading="loading">
            ...
            <el-row v-if="showRes">
              <el-col class="txt-right">
                <el-button :loading="posting" type="primary" @click="sureImport"
                  >确认导入</el-button
                >
              </el-col>
            </el-row>
            <!-- <el-row v-if="showRes">
              <el-col class="txt-right">
                <el-button :loading="posting"
                           type="primary"
                           @click="submit(false)">确认导入</el-button>
              </el-col>
            </el-row> -->
          </div>
...
export default {
  ...
  methods: {
    ...
    temDownload() {
      return importTemplate(this.importid, this.filename);
    },
    callServer() {
      const req = (query) => {
    ...
    submit(ignoreAbnormal) {
      this.posting = true;
      const me = this;
      submit(this.importid, this.tableParams.batchId, ignoreAbnormal)
        .then((data) => {
