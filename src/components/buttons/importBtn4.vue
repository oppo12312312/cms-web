<template>
  <el-button :type="btnType" :size="size" @click="openExceImportDialog">
    {{ name }}
    <v-dialog
    ...
    >
      ...
      >
        <div slot="tableHeader" class="">
          <v-form :fields="fields4" :fieldsData="tableParams3"> </v-form>
        </div>
      </basic-table>
    </v-dialog>
    <v-dialog
...
export default {
            ...
            return (
              <div>
                ...
                <el-button
                  ...
                  size="middle"
                  class={row.status !== 1 ? "txt-highlight" : "displayNone"}
                >
                  删除
                </el-button>
                <el-button
  ...
  methods: {
    ...
    tempRefresh() {
      return refresh(this.importCode, this.filename);
    },
    callServer() {
      const req = (query) => {
    ...
    callServer2() {
      return (query) => {
        return history(
          this.importCode,
          Object.assign({}, query, { businessId: this.batchid })
        );
      };
    },
    fecthData() {
    ...
    fecthData3() {
      this.$refs.basicTable3.fecthData();
    },
    beforeShow(data) {
      if (data.length) {
    ...
    async getTitles(importCode) {
      await titles(importCode).then((data) => {
    ...
    getProcess(id, cb) {
      ...
      progress(this.importCode, id).then(async (data) => {
        ...
        if (data.data.done) {
          await me.getTitles(me.importCode);
          this.showRes = true;
    ...
    async checkResult(item) {
      this.currentItem = item;
      await this.getTitles(item.importCode || this.importCode);
